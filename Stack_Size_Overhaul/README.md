<div align="center">

# Stack Size Overhaul

[![Version](https://img.shields.io/badge/v1.5-Version-0d1117?style=for-the-badge&labelColor=1a1e2e&logo=github&logoColor=white)]()
[![Author](https://img.shields.io/badge/AgentKush-Author-0d1117?style=for-the-badge&labelColor=1a1e2e&logo=steam&logoColor=white)]()
[![Type](https://img.shields.io/badge/EXMOD-Type-0d1117?style=for-the-badge&labelColor=1a1e2e&logo=databricks&logoColor=white)]()
[![Compatibility](https://img.shields.io/badge/All%20DLCs-Compatibility-0d1117?style=for-the-badge&labelColor=1a1e2e&logo=opensourceinitiative&logoColor=white)]()

Increases stack sizes on 1,262 items. Resources/ammo/ores 5x, everything else 3x. Max cap 9999

Requires **[JimK72's Icarus Mod Manager](https://github.com/Jimk72/Icarus_Software)**

</div>

---

## Changes

| Category | Multiplier | Examples |
|----------|-----------|---------|
| Resources/Ammo/Ores | **5x** | Fiber 200→1000, Stone 100→500, Arrows 100→500, Ore 50→250, Seeds 400→2000 |
| Building Pieces | **3x** | Floors/Walls 20→60, Furniture 5→15 |
| Consumables | **3x** | Bandages 10→30, Grenades 5→15 |
| Food/Drinks | **3x** | Berries 100→300, Meals 10→30 |
| All others | **3x** | Max cap 9999 |

328 items at 5x, 934 items at 3x. Unstackable items left at 1.

## Installation

Import `Stack_Size_Overhaul.EXMODZ` via Icarus Mod Manager.

## Version History

- **v1.5** - Removed a stale test-item entry (Item_Light_Test) that no longer exists in the game.
- **v1.4** - Added 6 new Limestone building pieces (curved walls, diagonal sets, advanced beams). Removed 2 dead refs (Item_Wood_Build_HalfNormal, Item_Wood_Build_HalfPitch). Total: 1,262
- **v1.1** - Added 36 new DLC items: Agave, Garlic, Onion, PricklyPear foods + trophies
- **v1.0** - Initial release, 1,232 items modified
---

<div align="center">

**Made by AgentKush** · [All Mods](https://github.com/AgentKush/Icarus-mods) · [Report a Bug](https://github.com/AgentKush/Icarus-mods/issues) · [Mod Manager](https://github.com/Jimk72/Icarus_Software)

*All mods are free. If you enjoy them, leave a star on the repo!*

</div>
